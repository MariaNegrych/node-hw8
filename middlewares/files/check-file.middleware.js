...
const {filesOptionsEnum, responseEnum: {BAD_REQUEST}} = require('../../constants')
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (filesOptionsEnum.PHOTO_MIMETYPES.includes(mimetype)) {
            ...
            if (size > filesOptionsEnum.MAX_PHOTO_SIZE) {
                return next(
                    new ErrorHandler(`Max photo size is ${filesOptionsEnum.MAX_PHOTO_SIZE}`, BAD_REQUEST)
                )
            }
            ...
            if (size > filesOptionsEnum.MAX_DOC_SIZE) {
                return next(
                    new ErrorHandler(`Max file size is ${filesOptionsEnum.MAX_DOC_SIZE}`, BAD_REQUEST)
                )
            }
